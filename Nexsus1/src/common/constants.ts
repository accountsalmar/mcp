...
// =============================================================================
// NARRATIVE PATTERN CONFIGURATION
// =============================================================================

/**
 * Narrative pattern configuration for context-aware embeddings
 *
 * Patterns define how records are converted to natural language narratives
 * for improved semantic search quality.
 */
export const PATTERN_CONFIG = {
  /** Directory where pattern JSON files are stored */
  PATTERNS_DIR: process.env.PATTERNS_DIR || 'data/patterns',
  /** Default number of sample records for pattern generation */
  DEFAULT_SAMPLE_COUNT: parseInt(process.env.PATTERN_SAMPLE_COUNT || '10', 10),
  /** Enable/disable pattern-based encoding (fallback to legacy if disabled) */
  ENABLED: process.env.PATTERN_ENCODING_ENABLED !== 'false',
  /** Maximum length of generated narrative text */
  MAX_NARRATIVE_LENGTH: parseInt(process.env.PATTERN_MAX_LENGTH || '2000', 10),
  /** Current pattern schema version */
  VERSION: 1,
} as const;
