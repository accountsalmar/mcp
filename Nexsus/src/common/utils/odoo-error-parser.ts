 ...
 * 2. Rights restriction: "You do not have enough rights to access the fields
 *    "field1,field2" on Model (model.name)"
 * 3. Compute errors: "Compute method failed to assign model(ids,).field_name"
 * 4. Singleton errors: "Expected singleton: model(1, 2, 4, ...)" - Odoo-side bugs
...
export function parseOdooError(errorMessage: string): OdooSecurityError {
  ...
  // Pattern 2: Rights restriction (Odoo 17+ format)
  // "You do not have enough rights to access the fields "field1,field2,field3" on Model Name (model.name)"
  const rightsPattern = /do not have enough rights to access the fields\s*"([^"]+)"\s*on\s*[^(]*\(([^)]+)\)/i;
  const rightsMatch = normalizedMessage.match(rightsPattern);
  if (rightsMatch) {
    result.type = 'security_restriction';

    // Extract comma-separated field names from quoted string
    const fieldList = rightsMatch[1];
    const fields = fieldList.split(',').map(f => f.trim()).filter(f => f.length > 0);
    result.restrictedFields = fields;

    // Extract model name from parentheses
    result.model = rightsMatch[2].trim();
    result.operation = 'read';

    return result;
  }

  // Pattern 3: Compute error
...
export function isFieldRestrictionError(errorMessage: string): boolean {
  ...
  const patterns = [
    ...
    /not have enough rights to access/i,  // "You do not have enough rights to access the fields"
