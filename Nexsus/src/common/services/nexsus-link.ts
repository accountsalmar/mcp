...
export async function resolveJsonFkLinks(
  ...
  for (const field of jsonFkFields) {
    // Get JSON FK mapping for this field (only FK type can be resolved)
    const mapping = getJsonFkMapping(modelName, field);
    if (!mapping) {
      console.error(`[NexsusLink] No JSON FK mapping for ${modelName}.${field}`);
      continue;
    }
    // Skip metadata fields - they don't have FK targets
    if (mapping.mapping_type === 'metadata') {
      console.error(`[NexsusLink] Skipping metadata field ${modelName}.${field} (not resolvable)`);
      continue;
    }
    // Ensure FK mapping has required target model ID
    if (!mapping.key_target_model_id) {
      console.error(`[NexsusLink] FK mapping ${modelName}.${field} missing key_target_model_id`);
      continue;
    }
