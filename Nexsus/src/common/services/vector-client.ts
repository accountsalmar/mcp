...
/**
 * Clear all data and graph points from the unified collection
 *
 * This deletes:
 * - All data points (point_type='data') - synced Odoo records
 * - All graph points (point_type='graph') - FK relationship edges
 *
 * Schema points (point_type='schema') are preserved.
 *
 * @returns Object with counts of deleted data and graph points
 */
export async function clearDataAndGraph(): Promise<{ data_deleted: number; graph_deleted: number }> {
  if (!qdrantClient) throw new Error('Vector client not initialized');

  const collectionName = UNIFIED_CONFIG.COLLECTION_NAME;

  // Count before deletion for reporting
  const dataCountBefore = await qdrantClient.count(collectionName, {
    filter: { must: [{ key: 'point_type', match: { value: 'data' } }] },
    exact: true,
  });

  const graphCountBefore = await qdrantClient.count(collectionName, {
    filter: { must: [{ key: 'point_type', match: { value: 'graph' } }] },
    exact: true,
  });

  console.error(`[Vector] Clearing ${dataCountBefore.count.toLocaleString()} data points...`);
  ...
  console.error(`[Vector] Clearing ${graphCountBefore.count.toLocaleString()} graph points...`);

  // Delete all graph points
  await qdrantClient.delete(collectionName, {
    filter: { must: [{ key: 'point_type', match: { value: 'graph' } }] },
    wait: true,
  });

  console.error(`[Vector] Cleared ${dataCountBefore.count.toLocaleString()} data + ${graphCountBefore.count.toLocaleString()} graph points`);

  return {
    data_deleted: dataCountBefore.count,
    graph_deleted: graphCountBefore.count,
  };
}
