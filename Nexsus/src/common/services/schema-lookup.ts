...
import { loadSchemaFromQdrant } from './schema-vector-loader.js';
 ...
 * Schema source type
 */
export type SchemaSource = 'excel' | 'qdrant';

/**
 * Current schema source
 */
let currentSchemaSource: SchemaSource = 'excel';
...
export function clearSchemaLookup(): void {
  ...
  schemaEmpty = false;
  initialized = false;
  currentSchemaSource = 'excel';
}

// =============================================================================
// QDRANT-BASED INITIALIZATION (Async)
// =============================================================================

/**
 * Initialize schema lookup from Qdrant vector database
 *
 * This loads schema directly from Qdrant (synced from Odoo) instead of
 * the Excel file. Use this after running `sync schema --source odoo`.
 *
 * Benefits:
 * - Always reflects current Odoo schema (no Excel file needed)
 * - Works in Railway deployments where Excel can't be updated
 * - Single source of truth (Qdrant)
 *
 * @returns Promise<boolean> - true if initialization succeeded
 */
export async function initializeSchemaLookupFromQdrant(): Promise<boolean> {
  console.error('[SchemaLookup] Initializing from Qdrant...');

  try {
    const schemas = await loadSchemaFromQdrant();

    if (schemas.length === 0) {
      console.error('[SchemaLookup] Warning: No schema data in Qdrant - validation will be skipped');
      initialized = true;
      schemaEmpty = true;
      schemaLookup = new Map();
      validModels = new Set();
      modelIdToName = new Map();
      currentSchemaSource = 'qdrant';
      return false;
    }

    // Build the lookup maps (same logic as initializeSchemaLookup)
    buildSchemaLookupMaps(schemas);

    currentSchemaSource = 'qdrant';
    console.error(`[SchemaLookup] Initialized from Qdrant: ${validModels!.size} models, source: ${currentSchemaSource}`);

    return true;
  } catch (error) {
    console.error('[SchemaLookup] Error initializing from Qdrant:', error);
    return false;
  }
}

/**
 * Build schema lookup maps from schema rows
 *
 * Shared logic between Excel and Qdrant initialization.
 */
function buildSchemaLookupMaps(schemas: NexsusSchemaRow[]): void {
  schemaLookup = new Map();
  validModels = new Set();
  modelIdToName = new Map();
  schemaEmpty = false;
  ...
  const totalFields = Array.from(schemaLookup.values()).reduce((sum, m) => sum + m.size, 0);
  console.error(`[SchemaLookup] Built maps: ${validModels.size} models, ${totalFields} fields (including FK variants)`);

  initialized = true;
}

/**
 * Get current schema source
 */
export function getSchemaSource(): SchemaSource {
  return currentSchemaSource;
}

/**
 * Refresh schema from Qdrant (async)
 *
 * Clears existing cache and reloads from Qdrant.
 * Use after `sync schema --source odoo` to pick up new fields.
 */
export async function refreshSchemaFromQdrant(): Promise<boolean> {
  console.error('[SchemaLookup] Refreshing schema from Qdrant...');
  clearSchemaLookup();
  return initializeSchemaLookupFromQdrant();
}
