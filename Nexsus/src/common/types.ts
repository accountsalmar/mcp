 ...
 * - 'fk': Foreign key fields where JSON keys are record IDs (build Qdrant UUIDs)
 * - 'metadata': Non-FK fields tracked for documentation (no UUID generation)
 */
export interface JsonFkMapping {
  ...
  /**
   * Mapping type:
   * - 'fk': Build Qdrant UUIDs for JSON keys (FK resolution enabled)
   * - 'metadata': Track only, no UUID generation (non-FK fields)
   * Defaults to 'fk' for backward compatibility
   */
  mapping_type?: 'fk' | 'metadata';
  /** Target model for JSON keys (required for 'fk' type) */
  key_target_model?: string;
  /** Target model ID in Odoo (required for 'fk' type) */
  key_target_model_id?: number;
  /** How to interpret JSON keys: 'record_id' for numeric IDs, 'string' for text keys */
  key_type?: 'record_id' | 'code' | 'string';
  /** How to interpret JSON values */
  value_type?: 'percentage' | 'amount' | 'count' | 'duration_ms' | 'boolean' | 'mixed';
