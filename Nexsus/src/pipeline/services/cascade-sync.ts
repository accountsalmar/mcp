...
import {
  ...
  parseOdooError,
} from '../../common/utils/odoo-error-parser.js';
import {
  upsertRelationship,
...
import {
  syncPipelineData,
  loadPipelineSyncMetadata,
  savePipelineSyncMetadata,
} from './pipeline-data-sync.js';
import { UNIFIED_CONFIG } from '../../common/constants.js';
import { buildDataUuidV2 } from '../../common/utils/uuid-v2.js';
import type { RelationshipType, PipelineDataPoint, SyncFkDependency } from '../../common/types.js';
 ...
 * 1. **Field restriction errors**: "You do not have enough rights to access the fields..."
 *    - Parse error to extract restricted field names
 *    - Retry with those fields excluded
 * 2. **Batch size errors**: OverflowError, MemoryError, XML-RPC limits
 *    - Halve the batch size and retry
 *    - Keep retrying until batch size reaches MIN_RETRY_BATCH_SIZE
 *
 ...
 * @returns Object with fetched records, skipped IDs, and excluded fields
 */
async function fetchBatchWithRetry(
  ...
  batchNum: number,
  totalBatches: number
): Promise<{ records: Array<Record<string, unknown>>; skippedIds: number[]; excludedFields: string[] }> {
  let currentBatchSize = batchIds.length;
  let currentIds = batchIds;
  let retryCount = 0;
  const allRecords: Array<Record<string, unknown>> = [];
  const skippedIds: number[] = [];
  let currentFields = [...odooFields];
  ...
  while (currentIds.length > 0 && retryCount < MAX_BATCH_RETRIES) {
    try {
      ...
      // Check if it's a field restriction error (can be handled by excluding fields)
      if (isFieldRestrictionError(errorMsg)) {
        const parsed = parseOdooError(errorMsg);

        if (parsed.restrictedFields.length > 0) {
          // Found restricted fields - exclude them and retry
          const newExcluded = parsed.restrictedFields.filter(f => !excludedFields.includes(f));
          excludedFields.push(...newExcluded);

          // Remove restricted fields from current fields
          currentFields = currentFields.filter(f => !excludedFields.includes(f));

          // Log the field restriction
          for (const field of newExcluded) {
            console.error(`[${modelName}] Field '${field}' restricted (${parsed.type}) - removed from query`);
          }

          // Calculate delay with jitter
          const delay = Math.floor(Math.random() * 1500) + 500;
          console.error(`[${modelName}] Retry ${retryCount}/${MAX_BATCH_RETRIES} in ${delay}ms (fields reduced to ${currentFields.length})`);

          await new Promise(resolve => setTimeout(resolve, delay));
          continue; // Retry with reduced fields
        }

        // Field restriction error but couldn't extract field names - skip batch
        console.error(`[CascadeSync] Batch ${batchNum}/${totalBatches}: Field restriction error but couldn't identify fields - skipping ${currentIds.length} records`);
        console.error(`[CascadeSync] Error: ${errorMsg.substring(0, 200)}`);
        skippedIds.push(...currentIds);
        break;
      }

      // Check if it's a batch size error (can be handled by reducing batch size)
      ...
      if (!isBatchSizeError) {
        // Non-retryable error - skip all IDs in this batch
      ...
      console.error(`[CascadeSync] Batch ${batchNum}/${totalBatches}: Batch size error, retrying with batch size ${currentBatchSize} (attempt ${retryCount}/${MAX_BATCH_RETRIES})`);
      ...
      // Process smaller batches (also need to handle field restrictions in sub-batches)
      for (const smallBatch of smallerBatches) {
        let subRetry = 0;
        let subFields = [...currentFields];
        ...
        while (subRetry < 3) {
          try {
            ...
            allRecords.push(...records);
            break; // Sub-batch succeeded
          } catch (subError) {
            const subErrorMsg = subError instanceof Error ? subError.message : String(subError);
            subRetry++;

            // Check for field restriction in sub-batch
            if (isFieldRestrictionError(subErrorMsg)) {
              const subParsed = parseOdooError(subErrorMsg);
              if (subParsed.restrictedFields.length > 0) {
                const newSubExcluded = subParsed.restrictedFields.filter(f => !excludedFields.includes(f));
                excludedFields.push(...newSubExcluded);
                subFields = subFields.filter(f => !excludedFields.includes(f));
                currentFields = currentFields.filter(f => !excludedFields.includes(f));

                for (const field of newSubExcluded) {
                  console.error(`[${modelName}] Field '${field}' restricted (${subParsed.type}) - removed from query`);
                }
                continue; // Retry sub-batch with reduced fields
              }
            }

            // Sub-batch failed for other reasons - add to skipped
            if (subRetry >= 3) {
  ...
  return { records: allRecords, skippedIds, excludedFields };
}
...
export async function syncSpecificRecords(
  ...
  const allExcludedFields: string[] = []; // Track fields excluded due to permission errors
  let currentOdooFields = [...odooFields]; // Mutable field list that updates as we discover restrictions
  ...
  for (let i = 0; i < recordIds.length; i += FETCH_BATCH_SIZE) {
    ...
    // Use retry helper with field exclusion and batch size reduction on error
    const { records: batchRecords, skippedIds, excludedFields } = await fetchBatchWithRetry(
      odooClient,
      modelName,
      batchIds,
      currentOdooFields, // Use the current field list (may have excluded fields from previous batches)
      batchNum,
      totalBatches
    );
    ...
    // If new fields were excluded, update our field list for subsequent batches
    if (excludedFields.length > 0) {
      for (const field of excludedFields) {
        if (!allExcludedFields.includes(field)) {
          allExcludedFields.push(field);
        }
      }
      // Update field list for subsequent batches
      currentOdooFields = currentOdooFields.filter(f => !excludedFields.includes(f));
    }
  ...
  if (allExcludedFields.length > 0) {
    console.error(`[CascadeSync] Excluded ${allExcludedFields.length} restricted fields: ${allExcludedFields.join(', ')}`);
  }
  ...
  try {
    ...
    // Update pipeline sync metadata (FIX: cascade sync was not updating metadata)
    try {
      const metadata = loadPipelineSyncMetadata();
      const existingCount = metadata[modelName]?.record_count || 0;
      const payloadFieldCount = Object.keys(transformed[0]?.payload || {}).length;

      metadata[modelName] = {
        last_sync_timestamp: new Date().toISOString(),
        record_count: existingCount + recordsSynced,  // Accumulate count for cascade syncs
        field_count: payloadFieldCount,
      };
      savePipelineSyncMetadata(metadata);
      console.error(`[CascadeSync] Updated metadata: ${modelName} now has ${metadata[modelName].record_count} records`);
    } catch (metadataError) {
      // Don't fail the sync if metadata update fails
      console.error(`[CascadeSync] Warning: Failed to update metadata: ${metadataError}`);
    }
