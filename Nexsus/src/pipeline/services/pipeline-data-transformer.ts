...
import { PIPELINE_CONFIG } from '../../common/constants.js';
import {
...
import { getJsonFkMapping } from '../../common/services/json-fk-config.js';
import type {
  PipelineField,
...
export function buildPayload(
  ...
  for (const field of payloadFields) {
    ...
    // For JSON fields, check if it's a JSON FK field and build Qdrant UUIDs
      ...
      // Check if this is a JSON FK field (only process FK type, skip metadata)
      const jsonFkMapping = getJsonFkMapping(modelName, field.field_name);
      if (
        jsonFkMapping &&
        (jsonFkMapping.mapping_type === 'fk' || !jsonFkMapping.mapping_type) &&
