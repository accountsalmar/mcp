...
import { buildGraphUuidV2, buildDataUuidV2, getRelationshipTypeCode } from '../../common/utils/uuid-v2.js';
import {
  transformPipelineRecords,
} from './pipeline-data-transformer.js';
import { PIPELINE_CONFIG, UNIFIED_CONFIG } from '../../common/constants.js';
 ...
 */
export function loadPipelineSyncMetadata(): PipelineSyncMetadata {
 ...
 */
export function savePipelineSyncMetadata(metadata: PipelineSyncMetadata): void {
  const metadataPath = path.resolve(process.cwd(), PIPELINE_CONFIG.METADATA_FILE);
...
export async function syncPipelineData(
  ...
  try {
    ...
    // =========================================================================
    // DATE FILTERING - Filter records by date period
    // =========================================================================
    ...
    //
    // For accounting models (account.move, account.move.line), uses the 'date'
    // field (accounting date - when transaction occurred).
    // For other models, uses 'create_date' (when record was created in Odoo).
    //
    // Format must be YYYY-MM-DD (validated by schema)
    // Note: Using >= and <= for inclusive date range
    // =========================================================================

    // Model-specific date field mapping for accounting models
    const MODEL_DATE_FIELD_MAP: Record<string, string> = {
      'account.move.line': 'date',  // Accounting date (when transaction occurred)
      'account.move': 'date',       // Accounting date (when transaction occurred)
      // All others default to 'create_date'
    };

    const dateFilterField = MODEL_DATE_FIELD_MAP[modelName] || 'create_date';

    if (options.date_from) {
      // Convert YYYY-MM-DD to full datetime for Odoo (start of day)
      const dateFromWithTime = `${options.date_from} 00:00:00`;
      domain.push([dateFilterField, '>=', dateFromWithTime]);
      console.error(`[PipelineSync] Date filter on '${dateFilterField}' FROM: ${options.date_from}`);
    }

    if (options.date_to) {
      // Convert YYYY-MM-DD to full datetime for Odoo (end of day)
      const dateToWithTime = `${options.date_to} 23:59:59`;
      domain.push([dateFilterField, '<=', dateToWithTime]);
      console.error(`[PipelineSync] Date filter on '${dateFilterField}' TO: ${options.date_to}`);
    }

    // Log combined date filter if both are set
    if (options.date_from && options.date_to) {
      console.error(`[PipelineSync] Syncing records where ${dateFilterField} is between ${options.date_from} and ${options.date_to}`);
    }
    ...
    // 11. Auto-create indexes for payload fields
    if (recordsUploaded > 0) {
      const payloadFieldsForIndex = await getPayloadFieldsFromSchema(modelName);
      if (payloadFieldsForIndex.length > 0) {
        const indexResult = await ensureModelIndexes(modelName, payloadFieldsForIndex);
        if (indexResult.created > 0) {
          console.error(`[PipelineSync] Created ${indexResult.created} indexes for ${modelName}`);
        }
      }
    }
...
export async function getPipelineSyncStatus(): Promise<{
  ...
  const pipelineSyncMetadata = loadPipelineSyncMetadata();
