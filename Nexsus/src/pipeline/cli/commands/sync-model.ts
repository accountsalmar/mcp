...
import { syncWithCascade, formatCascadeResult } from '../../services/cascade-sync.js';
import { ensureBaseIndexes } from '../../services/index-service.js';
import { modelExistsInSchema, getPayloadFieldsFromSchema } from '../../../common/services/schema-query-service.js';
import { clearPipelineSyncMetadata } from '../../services/pipeline-data-sync.js';
...
export async function syncModelCommand(
  ...
  try {
    ...
    // Ensure base indexes exist (point_type, model_name, etc.)
    const baseIndexResult = await ensureBaseIndexes();
    if (baseIndexResult.created > 0) {
      console.log(chalk.dim(`  Created ${baseIndexResult.created} base indexes`));
    }
  } catch (error) {
